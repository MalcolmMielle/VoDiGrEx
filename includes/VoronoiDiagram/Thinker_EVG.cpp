...
void AASS::Thinker_EVG::think(const cv::Mat& map_in)
{
	reset();
...
grid_type AASS::Thinker_EVG::read_file(const cv::Mat& map,
		     int unknown_min,
		     int unknown_max) {
...
AASS::topologicalmap::GraphList AASS::Thinker_EVG::getGraph() const
{
	
	/* convert skeleton to GraphList */
	topologicalmap::GraphList gl;
...
// 	vector<unsigned int> children; //!< if # children > 1, graph branches
	...
	std::vector< topologicalmap::Vertex > vvec;
...
// 		std::string& type, const cv::Mat& m, cv::Point2i p, Vertex& vertex_out
	...
	for(size_t i = 0 ; i < _skel.size() ; ++i){
		for(size_t j = 0 ; j < _skel[i].children.size() ; ++j){
			gl.addEdge(vvec[i], vvec[ _skel[i].children[j] ]);
		}
	}
